"""launch_gauntletruns_readpapers.py — Batch-run Gauntlet in paper reading mode.

For each subdirectory containing a PDF:
1. Uploads the PDF to Gemini and identifies 2 closest topics
...
5. Runs main.py with inputs/readpaper.pdf as call and the paper as proposal

Usage:
    python launch_gauntletruns_readpapers.py <papers_dir> \\
        --topics ../TOPICS.txt \\
        --baseline-config ../config_readpaper_archgeneric.toml \\
        --persona-dir personas/reading_assistant
"""
...
def create_custom_config(
    ...
    if persona_parts[0] == "personas":
        persona_subdir = "/".join(persona_parts[1:])  # e.g., "reading_assistant"
    else:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(f"# Auto-generated config for paper reading\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch-run Gauntlet in paper reading mode with dynamic persona selection"
    )
    parser.add_argument("papers_dir", type=Path,
                        help="directory containing paper subdirectories (each with a PDF)")
    parser.add_argument("--topics", type=Path, required=True,
                        help="path to TOPICS.txt file")
    parser.add_argument("--baseline-config", type=Path, required=True,
                        help="baseline config.toml to extend (e.g., config_readpaper_archgeneric.toml)")
    parser.add_argument("--persona-dir", type=Path, required=True,
                        help="directory containing persona .md files (e.g., personas/reading_assistant)")
    parser.add_argument("--readpaper-pdf", type=Path, default=Path("../inputs/readpaper.pdf"),
                        help="path to readpaper.pdf (default: ../inputs/readpaper.pdf)")
    ...
    # --- Validate paths ---
    if not args.readpaper_pdf.exists():
        sys.exit(f"ERROR: readpaper.pdf not found at {args.readpaper_pdf}")
    ...
    for subdir in sorted(subdirs):
        ...
        if not pdfs:
            ...
            continue
        if len(pdfs) > 1:
            print(f"          [skip] multiple PDFs found, ambiguous\n")
            skip_count += 1
            continue
        ...
        try:
            # Upload PDF and get matching topics
            gemini_file = upload_pdf_to_gemini(paper_pdf)
            ...
            custom_config = subdir / "config_readpaper.toml"
